        ...
        success: function (response) {
            ...
            window.onscroll = function () {
                ...
                if (scrollTop + windowHeight >= scrollHeight && personalMode == 1) {
                    //渲染新歌曲,一次10首
                    //response.ids 为喜欢的音乐UID数组
                    for (let i = likesReady; i < likesReady + 10 && response.ids[i]; i++) {
                        //发送ajax请求获取歌曲详情信息
                        ajax({
                            url: serverUrl + "/song/detail",
                            data: {
...
function mylikesPlayAll() {
    ...
    for (let i = 0; i < likes.length; i++) {
        musicAddFunction(
            ...
            JSON.parse(
                likes[i].getElementsByTagName("div")[0].getAttribute("data-music-songInfo")
            )
        )
    }
    musicPlayFunction(
        likes[0].getElementsByTagName("div")[0].getAttribute("data-music-id"),
        JSON.parse(likes[0].getElementsByTagName("div")[0].getAttribute("data-music-songInfo"))
    )
}
...
document.querySelector(".mylikes>ul").addEventListener("click", function (e) {
    ...
    const songInfo = JSON.parse(target.parentNode.getAttribute("data-music-songInfo"))
    // 判断是否匹配目标元素
